...
bool PdbFile::ReadSig(char* module_name, char* OUT pdb_signature)
{
	...
	char PEFileName[MAX_PATH] = { 0 };
	if (!GetModuleFileNameA(hModule, PEFileName, MAX_PATH))
	{
		return false;
	}
	...
	if (loopexit) {
		sprintf(
			pdb_signature, "%.2X%.2X%.2X%.2X%.2X%.2X%.2X%.2X%.2X%.2X%.2X%.2X%.2X%.2X%.2X%.2X%d",
