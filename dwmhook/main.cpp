...
__int64 __fastcall hkPresentMultiplaneOverlay(__int64 a1, char a2, int a3, signed int a4, __int64 a5, unsigned int a6, int* a7)
{
	...
	return orgFun(a1, a2, a3, a4, a5, a6, a7);
}
...
UINT WINAPI MainThread1(PVOID)
{
	...
	if (tempbuff != NULL) {
		Sleep(1000);
		 		if (VirtualFreeEx(GetCurrentProcess(), tempbuff, 0, MEM_RELEASE) == FALSE)
					AddToLog(XorStr("error"));
		 		else
					AddToLog( XorStr("free"));
	...
	PdbFile pdb;
	char sig_buff[100] = { 0 };
	if (pdb.ReadSig((char*)XorStr("dxgi.dll"), sig_buff) == false)
	{
		AddToLog(XorStr("ReadSig error"));
		return 0;
	}
	if (shared.shared_mem_ == NULL)
	{
		AddToLog(XorStr("shared mem  = NULL"));
		return 0;
	}
	...
	while (true)
	{
		...
		if (shared.shared_mem_->symbol_offset != 0)
		{
			...
			DWORD64 pvftable = (DWORD64)GetModuleHandleA(XorStr("dxgi.dll"));
			pvftable += (shared.shared_mem_->symbol_offset ^ 14238666);
			//AddToLog("symbol offset:%X", shared.shared_mem_->symbol_offset ^ 14238666);
			MemoryScanEx(GetCurrentProcess(), (BYTE*)&pvftable, 8, list);
			//AddToLog("vftable:%08X", pvftable);

			if (list.size() > 2)
			{
				AddToLog(XorStr("list size error"));
				return 0;
			}
			for (auto l : list)//过滤掉用来查找的地址,这里还需要改进。不能输出虚表的地址不然会搜索不到正确的地址
			{
				...
				{
					//AddToLog("list 0x%p\n", l);
					orgFun = (pf)vftable.Hook((PDWORD64)l, 23, (PDWORD64)hkPresentMultiplaneOverlay);
					oPresentDWM =  (PresentDWM_)vftable1.Hook((PDWORD64)l, 16, (PDWORD64)hkPresentDWM);
				}

			}
			//AddToLog(XorStr("hooked!\n"));
			return 0;
		}
	}
